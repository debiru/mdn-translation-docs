(function() {
  ...
  const NS = {
    ...
    BASE_URL_JA: 'https://developer.mozilla.org/ja/docs',
    regexFilters: {},
  };
  window.NS = NS;
  ...
  const Main = {
    ...
    filtersRow() {
      ...
      NS.filters.jaDoc.append(NS.pager.block);
    },
    main() {
      Sub.FetchManip.fetch(data => {
        NS.fullRecords = [];
        for (const dir of data.errors) {
          NS.fullRecords.push(Sub.Record.create(dir));
        }
  ...
  const Sub = {
    ...
    Record: {
      create(dir) {
        const self = {};
        self.enDoc = NS.BASE_URL_EN + dir;
        self.jaDoc = NS.BASE_URL_JA + dir;
      ...
      createRow(self) {
        ...
        tr.append(Sub.Record.createCell(cls[1], Sub.Record.link(self.enDoc)));
        tr.append(Sub.Record.createCell(cls[2], Sub.Record.link(self.jaDoc)));
        return tr;
      },
    ...
    URLManip: {
      ...
      exportQuery() {
        const assoc = {
          ...
          regex: NS.regexFilters.enDoc.regex.value,
          not_regex: NS.regexFilters.enDoc.not_regex.value,
        };
      ...
      loadQuery() {
        ...
        NS.regexFilters.enDoc.regex.value = param.get('regex');
        NS.regexFilters.enDoc.not_regex.value = param.get('not_regex');
      },
    },
    Main: {
      getValue(elem) {
